...
class LiveTrading:
    ...
    def place_order(self, symbol: str, action: str, quantity: int, price: float) -> Dict[str, Any]:
        ...
        try:
            ...
            order_id = f"LIVE_{int(now_ist().timestamp())}_{random.randint(1000, 9999)}"
            trade_value = quantity * price
            brokerage = paper_trading.calculate_zerodha_brokerage(trade_value, action)
            
            print(f"📊 LIVE TRADE: {action} {quantity} {symbol} @ ₹{price:.2f} | Order: {order_id} | Brokerage: ₹{brokerage:.2f}")
...
def get_current_prices(prefer_live: bool = True, instrument_type: str = 'stocks') -> Dict[str, float]:
    """Get current market prices for top symbols, prefer live for stocks."""
    symbols = get_top_symbols(instrument_type)
    return get_prices_for_symbols(symbols, prefer_live=prefer_live and instrument_type == 'stocks')
...
def is_market_open() -> bool:
    """Check if market is currently open using IST"""
    try:
        now = now_ist()
...
def _ensure_streamer():
    ...
    with _streamer_lock:
        if not _streamer_running:
            ...
            socketio.start_background_task(_market_streamer)
...
def wallet_balance():
    ...
    try:
        ...
        if trading_mode == 'paper':
            ...
            current_prices = get_current_prices(prefer_live=False)  # paper can stay simulated
...
def get_active_bots():
    ...
    try:
        ...
        for session in active_sessions:
            current_prices = get_current_prices(prefer_live=False)
            pnl_data = paper_trading.get_pnl(current_user.id, current_prices)
...
def get_market_watch_data():
    """Get market watch data. Pass ?type=stocks&live=1 to use live quotes."""
    try:
        instrument_type = request.args.get('type', 'stocks')
        live_flag = request.args.get('live', '0') == '1'
        
        symbols = get_top_symbols(instrument_type)
        prices = get_prices_for_symbols(symbols, prefer_live=live_flag and instrument_type == 'stocks')
        
        market_data_list = []
        for symbol in symbols:
            current_price = prices.get(symbol)
            previous_price = 1000 + (hash(symbol) % 5000)  # synthetic base for change calc
...
def start_bot():
    ...
    try:
        ...
        bot_config['session_id'] = session_obj.id
        ...
        thread = threading.Thread(
            ...
            args=(session_obj.id, bot_config, trading_session),
            name=f"BotThread-{session_obj.id}"
        )
        thread.daemon = True
        thread.start()
        ...
        trading_sessions[str(session_obj.id)] = trading_session
...
def stop_bot(session_id):
    ...
    try:
        session_obj = BotSession.query.get(session_id)
        if session_obj and session_obj.user_id == current_user.id:
            ...
            session_obj.force_stop = True
            session_obj.should_exit_positions = True
            ...
            session_obj.stopped_at = now_ist()
            session_obj.stop_requested = False
            session_obj.force_stop = False
            
            # Update final P&L
            pnl_data = paper_trading.get_pnl(current_user.id, current_prices)
            session_obj.pnl = pnl_data['net_pnl']
            db.session.commit()
...
def get_bot_performance(session_id):
    ...
    try:
        ...
        if session_obj.started_at:
            if session_obj.stopped_at:
                running_time = (session_obj.stopped_at - session_obj.started_at).total_seconds() / 3600
            else:
                running_time = (now_ist() - session_obj.started_at).total_seconds() / 3600
        
        performance = {
            'session_id': session_obj.id,
            'strategy': session_obj.strategy_name,
            'initial_capital': float(session_obj.initial_capital),
            ...
            'target_profit': float(session_obj.target_profit),
            'max_duration_hours': session_obj.max_duration_hours,
            'profit_target_achieved': pnl_data['net_pnl'] >= session_obj.target_profit if session_obj.target_profit > 0 else False,
            'time_remaining_hours': max(0, session_obj.max_duration_hours - running_time) if session_obj.status == 'running' else 0
        }
            ...
            if live_trading.initialize(settings.kite_api_key, settings.kite_access_token):
                ...
                if balance_data['success']:
                    return jsonify({
                        ...
                        'positions_count': balance_data.get('positions_count', 0),
                        'trades_count': 0,
                        'used_capital': balance_data['portfolio_value'] - balance_data['available_cash'],
                        'capital_usage_percent': ((balance_data['portfolio_value'] - balance_data['available_cash']) / balance_data['portfolio_value']) * 100 if balance_data['portfolio_value'] > 0 else 0,
                        'mode': 'live',
                        'note': 'Real Zerodha data'
                    })
                else:
...
def execute_enhanced_trade(session_id: int, config: Dict[str, Any], signal: Dict[str, Any]):
    ...
    try:
        ...
        current_prices = get_prices_for_symbols([signal['symbol']], prefer_live=(trading_mode == 'live'))
        current_price = current_prices.get(signal['symbol'], signal['price'])
        
        if signal['action'] == 'BUY':
            execution_price = round(current_price * 0.995, 2)
        else:
            execution_price = round(current_price * 1.005, 2)
        ...
        if trading_mode == 'paper':
            ...
            if result['success']:
                ...
                session_obj = BotSession.query.get(session_id)
                if session_obj:
                    session_obj.total_brokerage += result.get('brokerage', 0.0)
            ...
            # Live trading implementation (placeholder for actual order placement)
            pass
...
def run_enhanced_trading_bot(session_id: int, config: Dict[str, Any], trading_session: TradingSession):
    ...
    with app.app_context():
        try:
            ...
            while session_obj and session_obj.status == 'running':
                # THREAD-SAFE STOP CHECK - check both database and thread-safe flag
                if (trading_session.should_stop or 
                    not session_obj or 
                    session_obj.stop_requested or 
                    session_obj.force_stop or 
                ...
                if running_hours >= config['max_duration_hours']:
                    ...
                    session_obj.stopped_at = current_time
                    db.session.commit()
                    break
                ...
                if should_trade:
                    ...
                    current_prices = get_current_prices(prefer_live=False)  # strategies use simulated in paper mode
                    pnl_data = paper_trading.get_pnl(config['user_id'], current_prices)
                    ...
                    if signals:
                        ...
                        for signal in signals:
                            ...
                            session_obj = BotSession.query.get(session_id)
                            if not session_obj or session_obj.stop_requested or session_obj.force_stop or session_obj.status != 'running':
                        ...
                        if iteration % 10 == 0:
                            log_entry = Log(
                                user_id=config['user_id'],
                                message=f"Bot {session_id} running - P&L: ₹{pnl_data['net_pnl']:.2f}, Positions: {len(current_positions)}, Capital Used: {((config['capital'] - paper_trading.get_portfolio(config['user_id'])['available_cash']) / config['capital']) * 100:.1f}%",
                                level="DEBUG"
                            )
                            db.session.add(log_entry)
                            db.session.commit()
...
def handle_disconnect():
    ...
    if request.sid in _subscribers:
        del _subscribers[request.sid]
...
def handle_subscribe_market_data(data):
    ...
    if symbols:
        ...
        _subscribers[request.sid] = set(norm)
        emit('subscription_confirmed', {
            'symbols': norm,
            'message': f'Subscribed to {len(norm)} symbols',
            'timestamp': now_ist().isoformat()
        })
        _ensure_streamer()
...
def handle_request_market_data(data):
    """Request immediate one-shot market data for symbols"""
...
if __name__ == '__main__':
    with app.app_context():
        try:
            # ⚠️ Be careful: dropping all tables on every run will erase data.
            db.drop_all()
            db.create_all()
