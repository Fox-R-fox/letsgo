...
class PaperTrading:
    ...
    def calculate_zerodha_brokerage(self, trade_value: float, action: str) -> float:
        """
        Zerodha-like brokerage charges for intraday:
        ₹20 or 0.03% (whichever lower) + taxes/fees
        """
...
class LiveTrading:
    ...
    def place_order(self, symbol: str, action: str, quantity: int, price: float) -> Dict[str, Any]:
        """Place live order (stub with brokerage calc)"""
        try:
            if not self.kite:
                return {'success': False, 'error': 'Kite not initialized'}
    ...
    # NEW: live quotes for symbols (equities via NSE:<SYMBOL>, indices via known tokens)
    def get_market_quotes(self, symbols: List[str]) -> List[Dict[str, Any]] | None:
        try:
            if not self.kite:
                return None

            # Known index instrument tokens (Kite)
            index_token_map = {
                'NIFTY': 256265,        # NIFTY 50
                'BANKNIFTY': 260105,    # NIFTY BANK
                # Best-effort tokens for newer indices (may vary with env/instrument dump)
                'FINNIFTY': 12082530,
                'MIDCPNIFTY': 2880097,
            }
            ...
            for sym in symbols:
                s = sym.upper()
                if s in index_token_map:
                    instruments.append(index_token_map[s])
                else:
                    instruments.append(f"NSE:{s}")

            quotes = self.kite.quote(instruments)
            out = []
            # quotes is a dict keyed by instrument identifier
            for key, info in quotes.items():
                last_price = float(info.get('last_price', 0) or 0)
                ohlc = info.get('ohlc', {}) or {}
                close_ = float(ohlc.get('close', last_price) or 0)
                open_ = float(ohlc.get('open', last_price) or last_price)
                high_ = float(ohlc.get('high', last_price) or last_price)
                low_ = float(ohlc.get('low', last_price) or last_price)

                change = last_price - close_
                change_pct = (change / close_ * 100) if close_ else 0.0

                # Best effort to map original symbol back:
                symbol_guess = info.get('tradingsymbol') or str(info.get('instrument_token') or key)
                out.append({
                    'symbol': symbol_guess,
            ...
            print(f"⚠️ Live quotes fetch failed: {e}")
            return None
...
# ✅ UPDATED: Live market data (with OHLC) + fallback
@app.route('/api/market_watch_data')
@login_required
def get_market_watch_data():
    """Get market watch data (live via Zerodha if possible, else simulated)."""
    try:
        instrument_type = request.args.get('type', 'stocks')
        symbols = get_top_symbols(instrument_type)

        # Try live only during market hours and if creds are present
        settings = UserSettings.query.filter_by(user_id=current_user.id).first()
        can_try_live = (
            is_market_open() and
            settings and settings.kite_api_key and settings.kite_access_token and
            live_trading.initialize(settings.kite_api_key, settings.kite_access_token)
        )

        if can_try_live:
            live_data = live_trading.get_market_quotes(symbols)
            if live_data:
                return jsonify({'mode': 'live', 'data': live_data})
        ...
        for symbol in symbols:
            ...
            # Generate synthetic OHLC around current/base
            o = round(base_price * random.uniform(0.995, 1.005), 2)
            h = round(max(o, current_price) * random.uniform(1.000, 1.010), 2)
            l = round(min(o, current_price) * random.uniform(0.990, 1.000), 2)
            c = round(previous_price, 2)
        ...
        return jsonify({'mode': 'demo', 'data': market_data_list})
...
def stop_bot(session_id):
    ...
    try:
        ...
        if session_row and session_row.user_id == current_user.id:
            ...
            # IMMEDIATELY exit all positions (paper)
            current_prices = get_current_prices()
...
def execute_enhanced_trade(session_id: int, config: Dict[str, Any], signal: Dict[str, Any]):
    ...
    try:
        ...
        if trading_mode == 'paper':
            ...
            # Live trading implementation can be added here
            pass
    ...
    with app.app_context():
        try:
            ...
            while session_row and session_row.status == 'running':
                ...
                # Interruptible short sleep (3s total)
                for _ in range(30):
                    if trading_session.should_stop:
...
if __name__ == '__main__':
    with app.app_context():
        try:
            ...
            print("   - ✅ Enhanced Zerodha balance fetching")
